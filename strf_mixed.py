...
def get_strf(layerno):
    ...
    for i, data in enumerate(testloader, 0):
        ...
        feed = mel_norm(mix(a_specs, b_specs))

        feats = featurenet.feature(feat_data)
        a7, a6, a5, a4, a3, a2 = A_model(feats)

        Res_model.do_strf(feed, a7, a6, a5, a4, a3, a2)

        response = Res_model.xs[layerno - LAYER_NO_MIN][0] # use [0] because BS = 1: feeding one sample a time
        if len(layer_shape) == 4:
            layer_strfs[:,:,:] += response.reshape(chan, wid, hei, 1, 1) * feed.detach().numpy()
        else:
            layer_strfs[:] += response.reshape(wid, 1, 1) * feed.detach().numpy()
...
if __name__ == '__main__':
    ...
    import json
    json.dump(strf.tolist(), open("strf_mixed.json", "w"))